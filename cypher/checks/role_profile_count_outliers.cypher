...
// --- 1) Zusammenfassung: Median + Schwelle (95. Perzentil) + Anzahl Ausreisser, als
// selbsterklaerende Texte (nicht nur rohe Zahlen -- die KPI-Kachel zeigt sonst nur den Wert
// ohne Feldnamen) (-> KPI-Kacheln UI) ---
...
WITH collect(gesamt) AS alle, percentileCont(gesamt, 0.5) AS median, percentileCont(gesamt, 0.95) AS schwelle
UNWIND alle AS g
WITH median, schwelle, count(CASE WHEN g > schwelle THEN 1 END) AS ausreisserAnzahl
RETURN ('Median ' + round(median, 1) + ' / P95-Schwelle ' + round(schwelle, 1) + ' Rollen+Profile') AS verteilung,
       ausreisserAnzahl AS anzahl;
