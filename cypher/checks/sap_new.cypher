// Einzelberechtigungs-Check: Wer hat SAP_NEW?
...
// Aufruf:
//   ... cypher-shell -P "dataset => 'acme'" -P "asOf => date()" -f /cypher/checks/sap_new.cypher
//
// Risikofokus: SAP_NEW ueberbrueckt neue Berechtigungspruefungen nach Upgrades -- produktiv/
// dauerhaft zugewiesen hebelt es Sicherheitsverschaerfungen aus (Check A2).

// --- 1) Aufschluesselung nach Benutzertyp + Aktiv/Gesperrt ---
CALL {
  MATCH (u:User {dataset:$dataset})-[:HAS_PROFILE]->(:Profile {dataset:$dataset, id:'SAP_NEW'})
  RETURN u
  UNION
  MATCH (u:User {dataset:$dataset})-[a:ASSIGNED_TO]->(:Role)-[:HAS_PROFILE]->(:Profile {dataset:$dataset, id:'SAP_NEW'})
