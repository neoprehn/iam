...
| **Export CSV/PDF/…** | Exportiert **exakt die gerade sichtbare Ansicht** — Beschriftung und Format wechseln automatisch: Findings, Matches und Nutzerliste (jeweils echte Tabellen) gehen als **CSV**, mit denselben aktiven Filtern/derselben Sortierung wie auf dem Bildschirm; ohne aktive Filter der komplette Lauf. Rollen-Detail und Root-Cause (keine flache Tabelle) gehen als **PDF** über den Browser-Druckdialog („Als PDF speichern"). Findings-Export enthält auch die Regel-**Bezeichnung** (nicht nur die ID). In der **Einzelberechtigungen-/SoD-Übersicht** öffnet der Befehl stattdessen einen Dialog mit drei Formaten: **CSV**, **Excel** (native `.xlsx`, dezent eingefärbte Tabellenköpfe) und **ausführliches Excel** — Letzteres faltet je Zeile (Query bzw. SoD-Regel) die betroffenen Nutzer (ID/Name/Typ/Status) über Excels Gliederungs-/Gruppierungsfunktion darunter auf (eingeklappt, per Klick auf das „+" am linken Rand aufklappbar) und je Nutzer wiederum eine zweite, verschachtelte Ebene mit der/den belegenden Rolle(n)/Profil(en) — bei Einzelfiltern zusätzlich mit dem abgedeckten Berechtigungsobjekt bzw. der TCode-Prüfung. Bei sehr vielen Treffern (>200 Nutzer je Query) wird diese Rollen-/Objekt-Aufschlüsselung aus Performance-Gründen übersprungen (Hinweiszeile statt Daten; die Nutzerliste selbst bleibt vollständig) — für diese Fälle bleibt der interaktive Root-Cause je einzelnem User die Quelle. |
...
- **Risiko** — Risikoart/-stufe/-status (Dropdowns), Freitext zum potenziellen Risiko dieser
  Berechtigung, **Threat-Walkthrough** (Schritt-für-Schritt-Angriffspfad für die Threat-Analyse)
  sowie **Quellen** (Internetquellen/Referenzen, eine URL je Zeile).
