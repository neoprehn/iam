...
| `graphFile` | str | optional | Pfad zu einer eigenen `.cypher`-Datei (analog `cypherFile`), die statt der flachen Detailliste einen User→(Rolle→)Profil-Pfad liefert (feste Spalten `user`, `userType`, `userStatus`, `pathType` (`direkt`/`ueber Rolle`), `role` (nullable), `profile`). Wird über `POST /consistency-checks/{id}/graph` (Body `{dataset}`) ausgeführt; das Backend baut daraus generisch Cytoscape-Knoten/Kanten (`elements`-Array). Schaltet in der UI den bisher deaktivierten „Graph"-Pill scharf. Aktuell für A1–A3 umgesetzt (Pilot für die Tabelle/Graph-Umschaltung, s. `ROADMAP.md`); dieselbe Spaltenform ist auf weitere User→Profil-Checks übertragbar. |
