...
- [ ] **Umschalter Tabelle/Graph — echter Graph** — **erstes echtes Graph-Einsatzszenario**: Konfliktpfad **User → Rolle/Profil → Query → Regel** (nutzt die Evidenz-Kanten `VIA_ROLE`/`VIA_PROFILE`). Der Tabelle/Graph-Umschalter selbst ist da (s. o.), nur „Graph" ist noch deaktiviert. Einstieg ins **NVL/React**-Frontend (siehe nächster Punkt); Auswahl „als Graph oder als Tabelle anzeigen".
