...
- [ ] **Katalog-Auswahl (Filter/Regeln) — füllt Schritt ③ „Scoping" im Assistenten** (dort bisher nur
  Platzhalter): Browser über Queries (Einzelfilter) **und** SoD-Regeln, **filterbar** nach Kritikalität (z. B. nur very-critical), **Namensmuster** (z. B. `BC_*`), Modul, queryType. Mehrfachauswahl → Lauf nur über die Auswahl. *(Kritikalität/explizite Regel-IDs ✓ als Param; binärer Einzelfilter-Umfang „alle vs. nur SoD-relevant" ✓ im „Neuer Lauf"-Dialog, Details im Archiv; Muster-/Modul-Filter + Mehrfachauswahl-UI weiterhin offen.)*
