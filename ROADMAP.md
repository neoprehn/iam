...
- [ ] **„Fancy" Aufbereitung — gebrandetes NVL/React-Frontend.** **Ersetzt den temporären NeoDash-PoC** (Phase 6, Archiv). KPIs, **Graph-Darstellung der Konfliktpfade** (Neo4j Visualization Library / React) — visualisiert genau die Evidenz-Kanten (`VIA_ROLE`/`VIA_PROFILE`), Heatmap/Matrix, Drill-down. Die NeoDash-Karten-Cypher (`dashboards/sod_poc.json`) sind die Vorlage; NeoDash wird danach aus dem Compose entfernt.
