...
**Stand:** Phasen 0–3 und 5 abgeschlossen — Import (Can-Do, beide Pfade, performant), SoD-Evaluator (Zwei-Schritt, voll parametrisiert: User-Typ/Org/Sleeping/Scope), zwei Runner (`run_import`/`run_evaluate`). Phase 6 (NeoDash) als PoC abgeschlossen (Showcase-Stopp). **Phase 9 (transportable App — das Produktziel) gut unterwegs: Bau-Schritte 1–3 stehen** — Backend-API über die Runner, minimale UI mit **Import- und Auswerte-Assistent**, voller **Import im Container** (SE16-Konverter nach Python portiert; kein PowerShell mehr nötig). Bedienbar unter `http://localhost:8000/`. Offen: Phase 7 (Verteilung), restliche Phase 9 (Mandanten-Vergleich, Export, Backup/Restore, NVL/React), Phase X (Backlog), Phase 8 (Did-Do, Kür).
...
- [~] **Front-end — geführte Workflows** — **Bau-Schritt 2+3: Auswerte- UND Import-Assistent erledigt.** Schlanke statische Single-Page (`frontend/index.html`), **vom Backend ausgeliefert** (kein Node/React-Build, ein Container) unter `http://localhost:8000/`. **Import-Assistent** (System-Ordner aus `GET /import-folders`, Sprachen, Konvertierung/Schema überspringbar → `POST /imports`, Job-Fortschritt, frischt das Dataset-Dropdown auf). **Auswerte-Assistent**: datengetriebenes **Parameter-Formular statt JSON** (Ruleset, Stichtag, Nutzertyp-/Org-Profil, Sleeping, Mindest-Kritikalität, runId) aus `GET /profiles`; `POST /runs`, Schritt-Anzeige, **KPIs** (Findings/Regeln/sleeping), **Findings-Tabelle**, anklickbare Lauf-Liste (`(:Run)`). *Offen: gebrandetes NVL/React bleibt der „Fancy"-Schritt unten.*
...
- [~] **Backup/Restore/Clear** — **Clear erledigt.** `POST /datasets/{d}/clear` (ein Dataset inkl. dessen Runs/Findings/Matches) und `POST /reset` (alle Daten) — die **Ruleset-Schicht und das Schema bleiben in beiden Fällen** erhalten (so direkt neu importier-/auswertbar ohne Ruleset-Neuladen). Gebatcht über `apoc.periodic.iterate` (`cypher/admin/clear_dataset.cypher`, `cypher/admin/reset_data.cypher`); UI-Knöpfe „Dieses Dataset löschen" / „Alles zurücksetzen" mit Sicherheitsabfrage. Verifiziert: Reset → 0 Daten-Knoten, Ruleset 604 Queries intakt → Re-Import 1378/6816/182170. *Offen: Backup/Restore (`neo4j-admin database dump`/`load` je `dataset`).*
