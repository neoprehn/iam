...
- [~] **Multi-Varianten-Läufe.** Jede Variante (z. B. „Standard", „Übergreifend", „BUKRS=…") = ein eigener, **benannter** `(:Run)` (Titel-Feld, Lauf-Liste zeigt Titel als Hauptlabel + Run-ID als Mini-Chip). **Offen:** Org-Varianten sind aktuell **fix in `config/analysis_profiles.json`** definiert (`standard`/`uebergreifend`/`bukrs-*`) statt frei im Admin-Bereich konfigurierbar (eigene Org-Kriterien-Kombination + Name speichern); **paralleles Anlegen mehrerer Varianten in einem Schritt** im „Neuer Lauf"-Dialog (Mehrfachauswahl der Org-Varianten → je Variante ein Lauf, Untertitel = Variantenname) + Gruppierung als „Varianten-Set".
